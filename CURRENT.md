...
- PR #80 已加入首版 QQ 群聊 transport 并合入 `m7-c-long-run-graduation`：approved group + @Hikari + pure text + approved sender 才进入 `group:<group_id>` shared route；普通群噪声、富媒体、@其他成员、未授权群仍 fail-closed。
- M8-01 Shared Conversation Boundary 针对 #80 的真实多人边界缺口进行 post-merge hardening：`UserTurn` 新增 transport-grounded `actor_id` 与 `private/shared` scope，并贯穿 conversation wire、QQ spool、Conversation receipts；群成员权限拆为 `HIKARI_ONEBOT_ALLOWED_GROUP_USER_IDS`，group-only participant 不再自动获得私人 QQ 入口。
