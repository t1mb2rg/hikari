...
- Conversation Consolidation B 正在推进：standalone `hikari-conversation-host` 已切换到 `NaturalConversationEngine` + Jarvis production prompt + thin Natural Context，等待针对性验收；旧 `ConversationEngine.respond()` 的 heavy JSON grounding 已退出正式 Host 路径，但其公共 conversation lifecycle 仍被 Natural 引擎复用。
