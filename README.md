...
因此，普通仓库读取、项目文件修改、项目内命令执行、项目测试以及隔离 engineering branch commit 已经属于 Hikari 可直接完成的维护工作，不需要用户为每个文件、命令或测试逐步审批。Conversation 模型本身仍然没有直接 shell 或文件系统感知，实际执行由 Engineering Runtime 完成。
...
项目 mandate 委托的 non-protected engineering branch push 也已实现：只有单独的 narrow push turn 会把当前 EngineeringSession 的 engineering branch 推送到配置的 origin，branch/refspec 由 durable state 决定，不允许 force push、merge、protected branch、dirty worktree 或没有 committed delta 的空分支，普通维护 turn 仍然不获得 network/publish 权限。Draft PR 维护则仍是 capability gap——结果已被 mandate 委托，但实现尚未落地，因此仍会明确表示为 capability gap，而不是假装可用或要求用户逐动作授权。
